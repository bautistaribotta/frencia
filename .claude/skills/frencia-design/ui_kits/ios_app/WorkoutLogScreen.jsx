function WorkoutLogScreen({ onFinish }) {
  const { Card, Button, IconButton, StatTile, Stepper, SetRow, SegmentedControl, MetricPill, Badge, ProgressBar } = window.FrenciaDesignSystem_377129;
  ...
  return (
    <div style={{ padding: '56px 20px 120px', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          <span className="frencia-label">Ejercicio 1 / 5 · {ex.muscle}</span>
      ...
      {resting ? (
        ...
        <Card variant="elevated" style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <span className="frencia-label">RIR · repeticiones en reserva</span>
      ...
      {/* Sets log */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <span className="frencia-label">Series</span>
        {sets.map((s, i) => (
