# Frencia — Design System

> **Naming:** the product is **Frencia**. Any user-facing text, wordmark or icon must read **Frencia** — the earlier design-phase codename ("HEFT") is retired and must not appear anywhere.

> **Frencia** is an iOS training-log app. You bring the routine you already wrote on
> paper; Frencia turns it into a living record. It does **not** generate routines.
...
Frencia is a single product today: a **native-feeling iOS app**. Core jobs:
...
rebuilt the *skin* in Frencia's dark + green + orange language.
...
## CONTENT FUNDAMENTALS — how Frencia writes
...
Frencia uses **Lucide** (https://lucide.dev) — clean, consistent line icons at a
...
- **Don't** mix filled and line icons; don't recolor decoratively; don't use emoji
  ...
  Frencia marks completion with type + green check, not emoji.
...
**`components/`** — reusable React primitives (namespace `window.FrenciaDesignSystem_377129`)
